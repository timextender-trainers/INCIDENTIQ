...
public class PhoneTrainingController : Controller
{
    ...
    private readonly ISessionEvaluationService _sessionEvaluationService;
    ...
    public PhoneTrainingController(
        ...
        ISessionEvaluationService sessionEvaluationService,
        ILogger<PhoneTrainingController> logger)
    {
        ...
        _sessionEvaluationService = sessionEvaluationService;
        _logger = logger;
    }
    ...
    public async Task<IActionResult> StartCall(Guid scenarioId)
    {
        try
        {
            // If no valid scenario ID provided, create a new scenario
            if (scenarioId == Guid.Empty)
            {
                _logger.LogInformation("No valid scenario ID provided, creating new scenario");
                var newScenario = await _phoneScenarioService.CreateScenarioAsync(
                    "demo-user-123",
                    "Customer Service Representative", 
                    "TechCorp Solutions");
                scenarioId = newScenario.Id;
                _logger.LogInformation("Created new scenario {ScenarioId}", scenarioId);
            }
    ...
    public async Task<IActionResult> UserResponse([FromBody] GenerateResponseRequest request)
    {
        try
        {
            ...
            if (isClaudeAvailable)
            {
                ...
                PhoneCallSession? activeSession = null;
                
                // Try to get session by ID if provided
                if (request.SessionId.HasValue)
                {
                    activeSession = await _phoneScenarioService.GetSessionByIdAsync(request.SessionId.Value);
                    _logger.LogInformation("Retrieved session by ID: {SessionId}", request.SessionId.Value);
                }
                
                // Fallback to getting active session for the user
                if (activeSession == null)
                {
                    activeSession = await _phoneScenarioService.GetActiveSessionAsync("demo-user-123");
                    _logger.LogInformation("Retrieved active session for user demo-user-123");
                }
                
                // If still no session, create a temporary one for Claude to use
                if (activeSession == null)
                {
                ...
                if (activeSession != null)
                {
                    try
                    {
                        ...
                        // Check if conversation should end (6 turns reached)
                        bool isComplete = activeSession.Exchanges.Count >= 6;
                        
                        // If complete, end the session
                        if (isComplete)
                        {
                            await _phoneScenarioService.EndSessionAsync(activeSession.Id);
                            _logger.LogInformation("Session {SessionId} completed with {TurnCount} turns", activeSession.Id, activeSession.Exchanges.Count);
                        }
                        ...
                        var response = new ConversationResponse
                        {
                            ...
                            ResponseOptions = isComplete ? new List<object>() : GenerateContextualResponseOptions(hackerResponse, activeSession.Exchanges.Count),
...
<body>
    ...
    [HttpPost("EvaluateSession")]
    public async Task<IActionResult> EvaluateSession([FromBody] EvaluateSessionRequest request)
    {
        try
        {
            _logger.LogInformation("Starting AI evaluation for session {SessionId}", request.SessionId);

            // Get the completed session by ID
            var session = await _phoneScenarioService.GetSessionByIdAsync(request.SessionId);
            if (session == null)
            {
                return BadRequest(new { success = false, error = "Session not found" });
            }

            // Use SessionEvaluationService for comprehensive AI analysis
            var evaluation = await _sessionEvaluationService.EvaluateSessionAsync(session);
            ...
            { 
                ...
                evaluation = new
                {
                    ...
                    futureLearnings = evaluation.FutureLearnings?.Select(fl => new
                    {
                        title = fl.Title,
                        description = fl.Description,
                        estimatedTime = fl.EstimatedTime,
                        resourceType = fl.ResourceType,
                        priority = fl.Priority
                    }),
                    metrics = evaluation.Metrics != null ? new
                    {
                        completionTime = evaluation.Metrics.CompletionTime.ToString(@"mm\:ss"),
                        threatsDetected = evaluation.Metrics.ThreatsDetected,
                        totalExchanges = evaluation.Metrics.TotalExchanges,
                        responseTime = Math.Round(evaluation.Metrics.ResponseTime, 1),
                        grade = evaluation.Metrics.Grade
                    } : null,
                    securityBreaches = evaluation.SecurityBreaches?.Select(b => new
                    {
                        ...
                        impactExplanation = b.ImpactExplanation,
                        preventionAdvice = b.PreventionAdvice
                    }),
                    recommendations = evaluation.Recommendations?.Select(r => new
                    {
                        title = r.Title,
                        description = r.Description,
                        actionableAdvice = r.ActionableAdvice,
                        priority = r.Priority.ToString(),
                        roleSpecificContext = r.RoleSpecificContext
                    }),
                    riskAssessment = evaluation.RiskAssessment != null ? new
                    {
                        overallRiskLevel = evaluation.RiskAssessment.OverallRiskLevel.ToString(),
                        phishingResistanceScore = evaluation.RiskAssessment.PhishingResistanceScore,
                        primaryVulnerabilities = evaluation.RiskAssessment.PrimaryVulnerabilities,
                        riskProfile = evaluation.RiskAssessment.RiskProfile
                    } : null
                }
            });
        }
        ...
        {
            _logger.LogError(ex, "Error evaluating session {SessionId}", request.SessionId);
            return StatusCode(500, new { success = false, error = "Failed to evaluate session" });
        }
    }
